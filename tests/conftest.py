...
from khulnasoft_openapi import KhulnasoftOpenAPI, DefaultAioHttpClient, AsyncKhulnasoftOpenAPI
from khulnasoft_openapi._utils import is_dict
...
logging.getLogger("khulnasoft_openapi").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[KhulnasoftOpenAPI]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with KhulnasoftOpenAPI(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncKhulnasoftOpenAPI]:
